...
Then probe for `.claude/PLAN.md`. If it is absent — the normal state of
most projects — this command behaves in **every** respect exactly as it
always has: no grouping, no blocker flags, no extra reads, and **no
message about a missing plan**. Say nothing about it at all. Skip the
PLAN-AWARE GROUPING section below entirely and render the flat list.
