...
version: 0.3.0
type: command
description: Initialize the project's task backlog — creates .claude/TASKS.md, the .claude/tasks/ directory, and the external-LLM wiring under .claude/external/ (implement-prompt, tests-prompt, run-affected-tests.sh, run-full-tests.sh).
---
...
- Bash / PowerShell are only used to create directories
  (`mkdir -p .claude/tasks`, `mkdir -p .claude/external`) and to set
  the executable bit on the wrapper scripts (`chmod +x …`).
