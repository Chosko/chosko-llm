...
- **Home-path guard, same register.** Run `./scripts/check-home-paths.sh` after editing any shipped body under `commands/` or `skills/`. Silent on success, non-zero naming each file and line that cites another shipped file by an absolute install home. A shipped body cites another shipped file **by a path relative to itself**, counting directories up to the install home and back down — `./<file>.md` within one skill, `../<other>/…` from a file at a skill's root (`SKILL.md` or a supporting file beside it), `../../<other>/…` from a file under a skill's `references/`, `../skills/<other>/…` from a command — never `${CLAUDE_HOME:-$HOME/.claude}/…`, `$HOME/.claude/…` or `~/.claude/…`, which the executing agent always expands to the global home and which therefore misses every `--local` install, silently. No exceptions. A body asking whether an **optional** feature is installed at all does not use a path: it names the feature ("is the `claude-council` skill available?") and lets Claude resolve it across every scope — a path picks one scope and misses the other, silently. Guard only — bumps nothing itself. Detail in [docs/authoring-guide.md](docs/authoring-guide.md) § The home-path guard.
- **Routing guard, same register.** Run `./scripts/check-routing.sh` after adding, renaming, removing or re-kinding a pipeline feature, adding/dropping `requires: skill:pipeline-engine`, or editing `skills/pipeline-engine/references/routing.md`. Silent on success, non-zero naming violated invariant (every row names a shipped command/skill; every engine consumer has a row). Guard only — bumps nothing itself. Detail in [docs/authoring-guide.md](docs/authoring-guide.md) § The routing guard.
