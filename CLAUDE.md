...
- **Home-path guard, same register.** Run `./scripts/check-home-paths.sh` after editing any shipped body under `commands/` or `skills/`. Silent on success, non-zero naming each file and line that cites another shipped file by an absolute install home. A shipped body cites another shipped file **by a path relative to itself** — `./<file>.md` within one skill, `../<other>/references/<file>.md` from another skill's `SKILL.md`, `../../<other>/references/<file>.md` from another skill's reference file, `../skills/<other>/references/<file>.md` from a command — never `${CLAUDE_HOME:-$HOME/.claude}/…`, `$HOME/.claude/…` or `~/.claude/…`, which the executing agent always expands to the global home and which therefore misses every `--local` install, silently. One narrow exception: a **scope probe** — a body asking whether a feature is installed at all must check both scopes Claude Code loads from, so one of its paths is necessarily absolute; mark that line `scope-probe` and the guard permits it. Never a way to cite a file. Guard only — bumps nothing itself. Detail in [docs/authoring-guide.md](docs/authoring-guide.md) § The home-path guard.
