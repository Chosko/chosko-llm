...
orchestrator's only window onto the step. This is the one carve-out in WHAT
THIS SKILL READS AND WRITES' "it never reads a child's work": the reports
are already in hand, and no file is opened to get them. Under `--inline`
