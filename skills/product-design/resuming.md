...
Offered only when the marker says the process is complete. Establish what is
changing and why, conversationally — same register as the phases:
contribute, don't just extract — until the change can be stated as one
sentence naming the document and section it lands in. Then run `./amend.md`
with that sentence as its `<change>`: it pins, drafts, gates, writes and
compresses `design-process.md`, and its report and committing are the arm's.
The direct `amend "<change>"` argument form reaches the same file without
this menu.
