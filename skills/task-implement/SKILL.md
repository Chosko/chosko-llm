...
each agent reads `commands/follow-ups.md` at its installed path and applies
that command's rules to its own session, handing back at most three items. It
reads the rules rather than invoking the command — the run's closing call
stays the only one — and where the file is absent it skips the field silently
instead of failing. Nothing here or in `./delegated-runs.md` restates what
