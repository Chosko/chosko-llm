...
The fifth field is what makes that reading worth anything: each agent runs
`/follow-ups` on its own session before returning and hands back its list,
capped at three items. That is the command's own reading applied where the
task's context actually lives, so nothing here restates what counts as a
follow-up. Fold those lines into the run's list, attributed to the task they
came from. They are the agent's claim and the parent never saw
...
- Call `/follow-ups` per task in this session. The run's closing call is once
  per run, after the closing report and after the FEATURE COMPLETION proposal
  — see CLOSING THE RUN. A delegated agent running the command on its own
  session under `--agents` is not that call and is not an exception to this:
  it produces one of the closing call's inputs, in a session the parent never
  sees, and emits nothing to the user.
- Restate what counts as a follow-up, what is excluded, or how an item is
  written — here or in `./delegated-runs.md`. `commands/follow-ups.md` is their
  single authority; a second copy is a copy that will drift.
