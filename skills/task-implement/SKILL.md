...
The fifth field is what makes that reading worth anything: each agent may
return **at most three one-line follow-ups**, and only for something its task
left unrecorded on disk. Fold those lines into the run's list, attributed to
the task they came from. They are the agent's claim and the parent never saw
