...
unchanged, which bounds what the call can see: those four fields carry no
narrative, so an agent's own "this left something behind" never reaches the
parent. Read the launcher's conversation as the substantive half — the
delegation split, tasks skipped for unmet preconditions, failure lines,
declined feature slugs — and say so rather than implying the agents were
read for more than they returned. Where the user then asks to execute or plan a listed follow-up
