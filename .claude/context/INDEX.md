...
| [features.md](./features.md) | Shipped artifacts under `commands/`, `skills/`, `claude-md/`, `statusline/`, `hooks/`; frontmatter contract incl. optional `replaces:` / `requires:` and the hook-only `event:` / `matcher:`; the `description` contract (short what+when, flags in the body `#` header) and the loading-control keys `disable-model-invocation:` / `paths:` with the ten hidden features and the one path-scoped skill that carry them; the two non-invocable reference engines (`task-engine`, `pipeline-engine`), the routing guard and the home-path guard (`scripts/check-home-paths.sh`, enforcing that a shipped body cites another shipped file by a path relative to itself, and never by an install home — while a body asking whether an *optional* feature is installed names it instead of pathing it); the one revision surface (`pipeline-revise`, its change set and its four branch files); the two artifacts nobody invokes (`runbook-suggest`, `pipeline-suggest`); cross-refs to authoring guide. |
