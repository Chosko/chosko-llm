...
## [x] 7. Wire the archive rule into the pipeline-engine lint task
...
Done: commit e851347 (pushed) — .claude/TASKS.md task 178 Preconditions: 175, 176, 177 → 175, 176, 177, 196; one line changed, nothing else. Decisions: task 176 (graph.md), which step 6 noted also cites resolution.md, was deliberately not wired — the prompt scoped the edit to the lint task's line only; flagged for the user to rule on whether 176 needs the same precondition. No VERSION bump and no CHANGELOG entry: TASKS.md is backlog state, per the precedent of commit 4ace457. Premises: all held — ids 178 and 196 matched steps 2 and 6, and 178's line contained exactly 175, 176, 177.
