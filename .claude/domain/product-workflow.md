...
- **One gate, editorial question asked every time**, never inferred. `[NEW]` feature: no guard (no tasks), still asked. Sole exception: run as a `/pipeline-revise` owner step, the answer given at the revise gate is shown for explicit confirmation instead of re-asked.
