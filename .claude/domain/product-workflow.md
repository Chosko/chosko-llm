...
**Section 2's last field is Next, not readiness (task 154).** It names the ONE concrete action the feature needs, derived per feature from its `FEATURES.md` status, its task rollup and its readiness, and is exactly one of: `-` (`[DONE]` — nothing to do on a finished feature), `/task-add feature=<slug>` (`[NEW]` / `[ITERATED]`), `flip to [DONE] in FEATURES.md` (`[PLANNED]` w/ no task left that isn't `[DONE]`/`[SKIP]`, zero-task case included), `/task-implement <N>` (`[PLANNED]`, at least one such task, not blocked — `<N>` the lowest-numbered one), `blocked by <slug>[, <slug>]` (same case but blocked, naming every unsatisfied dependency). Blockedness suppresses ONLY `/task-implement`: planning ahead of a dependency and a bookkeeping flip are never blocked. Section 4 **echoes** that same value — the derivation lives in one place, so the recommendation can't contradict the row it recommends. Readiness itself is untouched: derived on every read, never stored, still what sections 3, 4 and 5 are built from. Only section 2's presentation of it changed.
