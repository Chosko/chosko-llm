...
- **One gate, editorial question decided when clear, asked when ambiguous.** Clear = mechanical signal, never confidence: task touched on its summary block, or nameable added scope → not editorial; no touched task, no added scope, no contract text changed → editorial. Clear case renders one `Classified: <editorial | not editorial> — <evidence>` line and writes w/o a reply. Ambiguous (a touched status needed a body read, borderline scope call, findings pointing different ways) → question asked, gate marks a recommended answer (no touched task and no added scope → editorial, else not editorial) w/ one evidence line; still needs explicit reply, silence still stops. `[NEW]` feature: no guard (no tasks), classified by same rule. Run as a `/pipeline-revise` owner step: the carried classification applied w/o a prompt when arm's own findings agree; confirmation form (w/ "This edit's own findings would mark …" line) when they disagree. `[IN PROGRESS]` touched-task refusal unchanged, fires first.
...
For each detected file `/task-add` enumerates the specific reconciliations that task needs to make to it — concrete points where implementation diverges from, or settles, what document says. What happens next follows from what could be named. Points nameable: one question per file, inside PHASE 3's existing single approval gate, never a second gate, offering three answers — **Grant** pre-authorisation for exactly those points, **Reference** the file as a read-only pointer authorising no edit, or **Drop** it and leave document to owner for later run. No points but path is that feature-derived task's own feature document (documentation task included): written as Reference, nothing asked — PHASE 4 feature case step 2 requires that pointer, so rule decides it. No points, other owned path, concrete reason to read it: asked with Reference and Drop only. No points, no read reason: dropped, nothing asked.

All three outcomes recorded in drafted body — grant as enumerated acceptance criteria plus dated, point-scoped `## Decisions` bullet; Reference as the path kept in Hints under one literal marker (`— read-only reference, do not edit`) and kept out of `Files:`, plus, when points were named, the same `## Decisions` note Drop writes; Drop as removal from Hints and `Files:` plus a `## Decisions` note naming what stays unreconciled. A marked Reference carried unchanged into a rewritten body — reconciliation, or `task-engine/references/amend.md` — counts as already decided and is not re-asked, which is what stops the next reconciliation stripping it. Approval that leaves question unanswered is re-asked, not assumed; PHASE 4 refuses to write a task whose detected file is neither granted, referenced nor removed.

Ownership still means one writer per artifact for the *pipeline commands*: `/task-add` itself never edits these files. A grant is one-off, dated, point-scoped exception carried in a task body and exercised by that task's *implementer*, not by `/task-add` — so "Who writes what" below still has exactly one writer per artifact. A Reference creates no writer at all: it is read authority only, which is why the rule can write one itself without asking.
