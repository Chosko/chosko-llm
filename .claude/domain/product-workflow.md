...
For each detected file `/task-add` enumerates the specific reconciliations that task needs to make to it — concrete points where implementation diverges from, or settles, what document says; file with no nameable point is dropped, no question asked. Then one question per file, inside PHASE 3's existing single approval gate, never a second gate, offering exactly two answers: grant pre-authorisation for exactly those points, or drop file and leave document to owner for later run. Both outcomes recorded in drafted body — grant as enumerated acceptance criteria plus dated, point-scoped `## Decisions` bullet; decline as removal from Hints and `Files:` plus a `## Decisions` note naming what stays unreconciled. Approval that leaves question unanswered is re-asked, not assumed; PHASE 4 refuses to write a task whose detected file is neither granted nor removed.

Ownership still means one writer per artifact for the *pipeline commands*: `/task-add` itself never edits these files. A grant is one-off, dated, point-scoped exception carried in a task body and exercised by that task's *implementer*, not by `/task-add` — so "Who writes what" below still has exactly one writer per artifact.
