...
- **One gate, editorial question asked every time**, never inferred. Gate marks a recommended answer from its own evidence — no touched task and no added scope → editorial, anything else → not editorial — w/ one evidence line; recommendation still needs explicit reply, silence still stops. `[NEW]` feature: no guard (no tasks), still asked. Sole exception: run as a `/pipeline-revise` owner step, the answer given at the revise gate is shown for explicit confirmation instead of re-asked; arm's own recommendation shows as one extra line only when it differs.
...
- **One gate**, nothing written before it by the skill or any arm. Shows verdict line, anchor/branch/tier, touched artifacts (untouched ones listed, overrulable), numbered owner steps, expected lint delta. **Editorial question asked every run**, never inferred — same rule `/architect amend` keeps; tier sets sequence length, never whether the question is asked. Judged tier marks the recommended answer — editorial → A, local or structural → B — w/ evidence line citing touched artifacts; still needs explicit reply. Editorial judged only when change is wording only, nothing downstream changes meaning, and A's sequence equals B's; insert/delete/reorder never editorial. The answer — user's reply, never the recommendation — carries into each `/architect amend` step, whose gate confirms it rather than re-asking.
