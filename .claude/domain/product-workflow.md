...
- **One gate**, nothing written before it by the skill or any arm. Shows verdict line, anchor/branch/tier, touched artifacts (untouched ones listed, overrulable), numbered owner steps, expected lint delta. **Editorial question classified when a mechanical signal settles it, asked only when not** — same clear/ambiguous split `/architect amend` keeps. Insert/delete/reorder never editorial, never asked. Amend: edge added/dropped, `Files:` change or scope change → not editorial; all three editorial conditions (wording only, nothing downstream changes meaning, A's sequence equals B's) → editorial; borderline wording-vs-meaning → asked, judged tier marks the recommended answer — editorial → A, local or structural → B — w/ evidence line citing touched artifacts; still needs explicit reply. Gate waits for a reply only when the question or the here-vs-runbook choice is open; neither → shows plan + `Classified:` line, actuates. Tier sets sequence length. The classification — user's reply when asked, automatic otherwise, never the recommendation — carries into each `/architect amend` step, which applies it w/o a prompt when its findings agree, confirms when they disagree.
...
- **Owner sequence upstream first** — no downstream step written against a state a later step changes. Amend's task step has two forms shown at the gate: per-task amends when `/architect amend` stales nothing; one `/task-add feature=<slug>` reconciliation (the only `[STALE]` clearer) when it stales a task or moves the feature `[ITERATED]` — no per-task amend for a staled task, unstaled tasks' own edits run first, conversation-only facts forwarded as the annotation text. A change naming another anchor's artifacts is scoped to the resolved anchor; the rest is a separate run. Owner refusing (touched `[IN PROGRESS]` task, a `[RUNNING]` runbook position) stops the sequence at that step; earlier writes kept and reported, never rolled back.
