...
an implementer builds against a stale task. The feature answers in two parts.
`/pipeline-check` reads every index and reports the inconsistencies
between them, each with the command that fixes it, so drift is visible before
production. `/pipeline-revise` takes the change itself, one item or a whole
change set: it walks each item's impact in both directions, verifies against
the indexes and the affected bodies, settles in advance every decision an
owner would make by rule, proposes one numbered plan behind one gate, and
then runs the owning commands in order — any step the user would rather
postpone deferred to a runbook from that same gate. A one-line auto-triggered suggester
...
surface owns nothing. A first-time pass through the pipeline remains cheaper
