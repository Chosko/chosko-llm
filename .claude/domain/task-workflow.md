...
shippable shape. The consumers cite it by a path relative to their own body —
`../skills/task-engine/references/<file>.md` from a command,
`../task-engine/references/<file>.md` from a file at another skill's root —
never an absolute install home, and never a `docs/` path, which does not
...
`[MISSING]`, `[STUBBED]`, `[INCORRECT]`, `[PARTIAL]`, `[IN PROGRESS]`, `[DONE]`, `[SKIP]`, `[STALE]`, `[PARKED]`.

`[DONE]` and `[SKIP]` only terminal statuses — only two `/task-clean` archives by default. `[PARKED]` is a task that asked a question under `/task-implement`'s `unattended` policy and had its work set aside — question in a trailing `## Parking handoff`, work-in-progress on `park/task-<N>` — until someone answers; written and unparked only by `/task-implement`, never in a default prune set, blocking its dependents like any unfinished task. Protocol lives in `task-engine`'s `parking.md`; design in [`./features/unattended-parking.md`](./features/unattended-parking.md). Vocabulary lives in the prompt layer alone; no shell script encodes it.
...
Feature document itself read-only to `/task-add`. Named in every feature-derived task's Hints; where the drafted task diverges from what it states, PHASE 3's design-change check (see [product-workflow.md](./product-workflow.md) § Documentation task) asks the user to agree to the design change, and the agreement — recorded dated in `## Decisions` — binds that task's *implementer* to update every passage stating the old design. Never `/task-add`, which stays a non-writer of every owned document. More generally, ownership guards decisions, never consistency: a *consequential edit* — a passage brought into agreement with an already-approved change, no meaning added — is in scope for `/task-implement` in any file, same commit, reported one line under *For the record*; new meaning in an owned document stays untouched and is named under *Follow-ups* as a precise follow-up (`/architect amend feature=<slug> "…"`). `/task-review` flags a documentation edit not traceable to the task, which is the check-after that replaces asking before.
...
On such run parent is **launcher**, not orchestrator. It resolves task list, evaluates delegation guard, hands each agent a prompt, records what comes back — nothing else. The three fields the guard needs (`Target:`, `Status:`, `Feature:`) all sit in each task's `TASKS.md` summary block, which PRE-FLIGHT already reads once for whole run, so **parent opens no `.claude/tasks/<N>.md` for a delegated task, on any path**. Hand-off prompt is fixed size — same frame every time with a different number in it: task number, repo's absolute path, run's resolved flags (open list, so a flag added later rides through without prompt growing; UNATTENDED among them, plus a held answer when the task is `[PARKED]`), and instruction for agent to read body, CLAUDE.md and `.claude/context/` itself. Under attended, an agent's question is its own result case: the launcher relays it to the user in the runbook's fixed block and sends the answer back to the same agent; under UNATTENDED the agent parks the task and returns `[PARKED]` with its question, which the launcher records before spawning the next — never a halt ([`./features/unattended-parking.md`](./features/unattended-parking.md)). What returns is exactly six values, the last two optional: task number, terminal status, commit hash (or that nothing was committed), one-line failure reason only when it failed, at most three follow-ups the agent derives by applying `/follow-ups`' rules to its own session, omitted when empty, which is almost every task, and at most three *For the record* lines for the parent's closing report, omitted when there are none. The field applies that command's rules, not a format of its own — what counts and what is excluded stay stated once, in the command's own body, and only the cap and the omit-when-empty belong to the contract. Agent is given the command's **name, never a path** — it receives a prompt rather than a file, so it has no anchor for a relative path, and an absolute one misses every `--local` install. Agent **reads** those rules and never invokes the command: the rules are applied once per run, in the closing report, and a read degrades where an invocation would not — no file to read means the field is skipped silently, never a failed task. It feeds the closing report's *Follow-ups* group and nothing else; parent records it, never acts on it mid-run and never verifies it, so it is never a reason to halt a run.
...
`--review` / `--rounds N` propagate to each implementor as part of run's resolved flags, and **each implementor spawns its own reviewer** — launcher → implementor → reviewer, one level deeper than the launcher alone. Parent passes flags through and nothing else changes there: it still sees only the returned fields (task number, terminal status, commit hash, one-line failure reason on failure, the agent's capped `/follow-ups` list and its capped *For the record* lines), never a finding, never a triage verdict — no rule is needed to keep findings out, because `/follow-ups`' own exclusion rule already does; what its reading catches is a deferral `/task-iterate` noted should become a task and that never did. Implementor's reviewer returns asynchronously like any other agent, so implementor must not commit before its own reviewer's result has arrived — a commit on the strength of a spawn call's return value is unreviewed work reported as reviewed.
...
Proposals are batched to the end of the run, never asked per-task — a many-task run that finishes several features asks once, for all of them together, after the last requested task's Step 7 (or Step 6, under `--no-commit`). A single-task run reaches "end of run" immediately after that one task, which is what makes "propose when it's the last task of a feature" and "propose only at the end of a batch" the same rule. Batching stops short of a run nobody is watching: a delegated agent under `--agents`, or a runbook step carrying `OPERATING RULES`, proposes nothing at all and names its candidates in its report, because its "end of run" is one task and its question would either go into the void or come back as a relayed block that halts the runbook — the outermost run is the only one that asks.
...
**The number is the handle the user replies with**, the way `/follow-ups`' numbering is: it starts at 1 in every report, carries no meaning beyond that, and a report with a single item still numbers it; a number that names a parked task's question is that task's answer, and the next run unparks it. `/runbook-run` closes in the same two groups with one *For the record* line per step (see [`./features/runbook-suite.md`](./features/runbook-suite.md)); `/task-review`'s report keeps *Needs you* then *For the record*, the second in the same one-line shape.

Order at the end of a run: the `[DONE]` feature-completion proposal, then this report, the run's last act. Under `--agents` the parent renders *For the record* from the sixth field of each agent's return and *Follow-ups* from the fifth plus every `[PARKED]` return's question, each attributed to its task; the rest of the list is the parent's own, since a delegated agent decides nothing. `requires: command:follow-ups` stays, since the rules must be installed to be read; with the command absent the list holds the run's own items only, silently.
