...
Every run of `/task-implement` ends with one closing report — at completion, at a failure halt and at a stop the user asked for alike — in two groups, in this order. **Needs you**: every item awaiting a decision, numbered `1.`, `2.`, …, each at whatever length it needs — an unresolved `BLOCKING` finding, a task left `[IN PROGRESS]` and why, a follow-up naming an owner's command with its anchor and passages, a precondition that no longer held, a feature slug declined at the proposal. **For the record**: one line per item, `<what deviated> — <why> — <resolved by whom>` — a criterion overshot, a wrong premise in the body, a consequential edit outside `Files:`, the `--no-commit` reminder that nothing was committed. Nothing in the second group is a question and nothing in it runs past one line; an empty group prints its heading and `none`. The one line in another shape is the round's resolved review pair.

**The number is the handle the user replies with**, the way `/follow-ups`' numbering is: it starts at 1 in every report, carries no meaning beyond that, and a report with a single item still numbers it. `/task-review` closes its own report in the same two groups with the same numbering, so a review's end and a run's end read alike, and `/runbook-run` closes in them too with one *For the record* line per step (see [`./features/runbook-suite.md`](./features/runbook-suite.md)).

Order at the end of a run: the `[DONE]` feature-completion proposal, then this report, then the single `/follow-ups` call. Under `--agents` the parent renders *For the record* from the sixth field of each agent's return, attributed to its task; *Needs you* is the parent's own, since a delegated agent decides nothing.
