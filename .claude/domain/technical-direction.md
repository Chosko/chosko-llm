...
`scripts/check-changelog.sh`, which fails when a `VERSION` bump carries no
matching `CHANGELOG.md` section. It is invoked by Claude at authoring time
— not by CI, and not by any command or skill.
