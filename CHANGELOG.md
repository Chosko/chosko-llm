...
- **The read-only and lint commands recognise a parked item.** `/task-list` marks a `[PARKED]` task with a trailing `⚠ parked`, in the slot `⚠ stale` uses, and `PARKED` filters like any status. `/task-clean` never puts `[PARKED]` in a default prune set; named explicitly, the plan says the prune would strand the task's question and orphan its `park/task-<N>` branch, and confirms first. `/pipeline-check` gains two findings — L12, a `[PARKED]` task whose body has no `## Parking handoff` (`ERROR`, fix `/task-implement <N>` in an attended session, which unparks or reports), and L13, a runbook whose `[P]` steps and index `Parked:` line disagree in either direction (`WARNING`, fix `/runbook-run <id>`, which rewrites the index from the body); neither probes whether a branch exists. Those two are the catalogue's only body reads, each bounded by the index — the bodies of the `[PARKED]` tasks, the bodies of the runbooks not `[DONE]` — and still never the archive, a feature document or a design document. `/task-add feature=<slug>` reconciliation classifies a `[PARKED]` task as it classifies a `[MISSING]` one: updated in place, its handoff and branch stay as they are and it stays `[PARKED]`; skip-and-replace names the parking branch in the reason and deletes it, local and remote. The routing table's `/task-implement` and `/runbook-run` rows carry the parking flags and the lines they write.

## 1.72.0 — 2026-09-23

- **`/runbook-run` and `/task-implement` close with one Follow-ups list.** Both closing reports are now *For the record* first, then *Follow-ups* — last, nearest the prompt, since its numbers are the reply handle. The *Needs you* group and the separate `/follow-ups` call after the report are gone: everything the first held (the flip question, a failed step or task, a `[~]` step, a task left `[IN PROGRESS]`, unresolved `BLOCKING` findings, declined slugs, steps or tasks left outside the range, every parked item with its question verbatim and multi-line) and everything the call yielded (the `/follow-ups` command's rules applied to the run's reading — the body read by name, never invoked, as delegated agents already do) go into one numbered list, two items naming the same action merged with the command form kept. Replying by number is ordinary conversation; a number naming a parked item's question is its answer. `requires: command:follow-ups` stays, since the rules must be installed to be read; with the command absent the list is the run's own items, silently. The delegated agent's fifth return field is unchanged and feeds the same group. `/follow-ups` itself is unchanged and states that a run-closing skill may render its rules this way.
- **`/task-add`'s owned-document `## Decisions` template** names *Follow-ups*, not *Needs you*, as where an implementer files a design decision met at implementation time.

## 1.71.0 — 2026-09-23

- **`/runbook-run` parks a step under the `unattended` policy.** The policy is one value per run: `--attended` or `--unattended` when passed (both together is an argument error), else the header's `Execution policy:` line, else `attended`; a header value outside the two words is an argument error naming it, and `--skip-parked` requires `--unattended`. All three compose with `--inline`. Under `attended` nothing changes — a question is relayed and the run waits. Under `unattended` a `QUESTIONS FOR USER` result takes a fifth row instead of the relay: the step is marked `[P]`, the block's question and options go into its `Context:` verbatim (never an approval-gate draft), the question is printed in chat under a number handle, the index gains `Parked: steps <ids>`, the step is committed like any other, and the run goes on to the steps that do not depend on it. When steps remain, none is selectable and one is `[P]`, that is not a deadlock: the run ends with the index at `[PENDING]` and the closing report names each parked step with its question and the steps waiting on it. At launch, unless `--skip-parked`, one numbered block pre-asks every `[P]` step in range — answered by number or `skip`, approval-gate items skip-only — and each answer unparks its step at once (`unparked with answer:` bullet, `[ ]`, one bookkeeping commit), as does a reply by number or step name mid-run or after the report; an unparked step is the next step selected. An answer to a number never printed, or a second answer to an answered step, is rejected with one line. An attended run that reaches a `[P]` step left by an earlier run asks it in the fixed relay block. All of it lives in a new `references/parking.md`, read only under `unattended` or when a `[P]` step is met; the spawned preamble carries one sentence declaring the run unattended, and the subagent and inline contracts gain two rules conditional on it — leave the tree clean of your own changes before asking, and answer the invoked skill's question from an `unparked with answer:` bullet rather than relaying it back.

## 1.70.0 — 2026-09-23

- **The runbook schema gains a parked step.** `runbook-schema.md` now has five step markers: `[P]` — parked, a `Context:` bullet `- <date> parked: <question>` carrying the question verbatim, options included, never an approval-gate draft — counted as not done in `Steps:`, committed like `[x]` and `[!]`, never beside `[~]`, and not a skip: the step is still to run, and `- <date> unparked with answer: <text>` plus `[ ]` is how it comes back. The index block gains `Parked: steps <ids>`, present only while a step is `[P]` — the `Failed at:` rule applied again, so a listing shows a runbook waiting on an answer without opening its body — and the header gains an optional `Execution policy: attended|unattended` line (absent means `attended`; any other value is an argument error at run time). `/runbook-create` writes that line only when asked for an unattended runbook, never by default, and never writes `[P]` or the two parking bullets; `/runbook-list` renders `Parked:` as a `↳ parked: steps <ids>` continuation exactly as it renders `Failed at:`, and `/runbook-describe` shows `[P]` in its marker column, the same continuation under its heading, and "parked" in its closing count — both still read-only, correcting nothing. What a run does with the marker is a following change.

## 1.69.1 — 2026-09-23

- **A delegated `/task-implement` agent asks through its launcher, or parks.** The fixed-size agent prompt no longer tells the agent it cannot ask and must stop on a human decision; it carries UNATTENDED with the other resolved flags, and a held answer for a `[PARKED]` task when the launcher holds one. Under attended, a question that genuinely needs the user — an unsettled acceptance criterion, a review round's approval gate — ends the agent's turn under `QUESTIONS FOR USER`; the launcher renders it in the runbook's fixed relay block with a task heading (`Task <N> — <title> — the agent is asking (round <r>)`, options, recommendation, any draft verbatim), waits for the answer, sends it to the same agent and repeats — never answering on the user's behalf. Under UNATTENDED the agent parks the task instead and returns `[PARKED]` with the handoff's question, which the launcher records, prints under the run's next handle number and moves past — a parked task is not a failure and does not halt the run; a `[PARKED]` task with no held answer gets no agent and is skipped with one line. A reply by number in chat is handled by the launcher as by an in-context run, the answered task's agent spawned next. The delegation guard is unchanged.

## 1.69.0 — 2026-09-23

- **`/task-implement --unattended` parks a task at a question instead of halting the run.** UNATTENDED is true when the flag was passed or the conversation declares the run unattended — the sentence a runbook step's preamble or a delegated-agent prompt carries; a merely non-interactive run is not that. Under it a question about the work parks the current task per `task-engine`'s `parking.md` — handoff written, work-in-progress on `park/task-<N>`, question printed in chat with a number handle — and the run continues with the next task; every prompt with a default takes it, each one a *For the record* line, and an ambiguous test runner aborts. At launch, unless `--skip-parked`, one numbered block lists every `[PARKED]` task in the resolved list with its question verbatim, answered by number or `skip` (approval gates skip-only), the answers held in run memory; a reply by number in chat during the run is that question's answer and moves its task to the front. A `[PARKED]` task reached with an answerer — an attended session, or a held answer — is unparked and resumed at the step its handoff names; with none it is skipped with one line. `--skip-parked` requires `--unattended`; `--unattended` is refused beside `--no-commit` and on a project whose `CLAUDE.md` maps another VCS. `parking.md` is read only under UNATTENDED or when a resolved task is `[PARKED]`; an attended run behaves exactly as before.
